...
class OrderTakingAgent():
    ...
    def get_response(self,pipe,tokenizer,messages):
        ...
        system_prompt = """
            ...
            * Don't tell the user to go to place to get the order
            ...
            produce the following output without any additions, not a single letter outside of the structure json below.
        ...
        # double check json 
        chatbot_output = ne_double_check_json_output(pipe,tokenizer,chatbot_output)

        output = self.postprocess(pipe,tokenizer,chatbot_output,messages,asked_recommendation_before)
        print("order",output)
        return output

    def postprocess(self,pipe,tokenizer,output,messages,asked_recommendation_before):
        ...
        if not asked_recommendation_before and len(output["order"])>0:
            recommendation_output = self.recommendation_agent.get_recommendations_from_order(pipe,tokenizer,messages,output['order'])
